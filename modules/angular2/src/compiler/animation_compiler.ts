...
var _ANIMATION_STATE_STYLES_VAR = o.variable('stateStyles');
...
class _AnimationBuilder implements AnimationAstVisitor {
  ...
  visitAnimationStateDeclaration(ast: AnimationStateDeclarationAst, stateMap: _AnimationBuilderStateMap): void {
    ...
    ast.styles.styles.forEach(entry => {
      StringMapWrapper.forEach(entry, (key, value) => {
        flatStyles[key] = value;
      });
    });
    var styleMap = [];
    StringMapWrapper.forEach(flatStyles, (key, value) => {
      styleMap.push([key, o.literal(value)]);
    });
    stateMap.registerState(ast.stateName, o.variable(_normalizeStateVariableName(ast.stateName)).set(o.literalMap(styleMap)).toDeclStmt());
  }
  ...
  visitAnimationStateTransition(ast: AnimationStateTransitionAst, stateMap: _AnimationBuilderStateMap): any {
    ...
    return new o.IfStmt(
      ...
      .and(_compareToAnimationStateExpr(_ANIMATION_NEXT_STATE_VAR, ast.toState)), [
        _ANIMATION_STATE_STYLES_VAR.set(o.variable(_normalizeStateVariableName(ast.toState))).toStmt(),
        _ANIMATION_FACTORY_RENDERER_VAR.callMethod('setElementStyles', [
          o.variable(_normalizeStateVariableName(ast.toState))
        ]).toStmt(),
  ...
  visitAnimationEntry(ast: AnimationEntryAst, stateMap: _AnimationBuilderStateMap): any {
    ...
    statements.push(_ANIMATION_STATE_STYLES_VAR.set(EMPTY_MAP).toDeclStmt());
    ...
    StringMapWrapper.forEach(stateMap.states, (value, stateName) => {
      if (!isPresent(value)) {
        value = o.variable(stateName).set(EMPTY_MAP).toDeclStmt();
      }
...
class _AnimationBuilderStateMap {
  private _states: {[key: string]: o.Statement} = {};
  get states() { return this._states; }
  registerState(name: string, value: o.Statement = null) {
